...
Создание объекта класса Ultrasonic:

.. code::

    sonar = Ultrasonic( trigger=pin15, echo=pin14, sclk=pin13 )

Если Вы не указываете входящие контакты (используйте данные контакты):
...
.. py:exception:: StopIteration

    Вызывается при возникновении ошибок при чтении байт от датчика. Возвращает -1
